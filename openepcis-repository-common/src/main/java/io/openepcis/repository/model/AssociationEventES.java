...
@AllArgsConstructor
@NoArgsConstructor
public class AssociationEventES extends EPCISEventES {
    ...
    public AssociationEventES(final AssociationEvent associationEvent, Map<String, Object> metadata)
            throws ValidationException {
        this(associationEvent, metadata, associationEvent.getContextInfo());
    }

    public AssociationEventES(final AssociationEvent associationEvent, Map<String, Object> metadata, List<Object> context)
            throws ValidationException {
        super(associationEvent, metadata, context);
        this.setAction(associationEvent.getAction());
        if (isNotEmpty(associationEvent.getChildQuantityList())) {
            this.setChildQuantityList(
                    associationEvent.getChildQuantityList().stream()
                            .map(item -> new QuantityListES(item, context))
                            .toList());
        }
        this.setParentID(EPCFormatUtil.buildEpc(associationEvent.getParentID(), context));
        this.setChildEPCs(
                EPCFormatUtil.buildListOfEpcES(associationEvent.getChildEPCs(), context));
        if (isNotEmpty(associationEvent.getBizTransactionList())) {
            this.setBizTransactionList(
                    associationEvent.getBizTransactionList().stream()
    ...
    @JsonIgnore
    @Override
    public AssociationEvent getCoreModel() {
        return AssociationEvent.associationEventBuilder()
                ...
                .parentID(
                        EPCFormatUtil.getEpcInExpectedFormat(
                                this.getParentID(), EPCFormat.Always_GS1_Digital_Link.name()))
    ...
    @JsonIgnore
    @Override
    public AssociationEvent getCoreModel(FormatPreference preference) {
        return this.getCoreModel(preference, getContextInfo());
    }

    @JsonIgnore
    @Override
    public AssociationEvent getCoreModel(FormatPreference preference, List<Object> context) {
        ...
        return AssociationEvent.associationEventBuilder()
                ...
                .bizTransactionList(
                        EventConvertor.getCoreModel(this.getBizTransactionList(), preference, context))
                .certificationInfo(this.getCertificationInfo())
                .build();
    }
