...
@RolesAllowed("query")
@RegisterRestClient(configKey = "bizsteps-api")
public interface BizStepsTopLevelApi {
            ...
            value = {
                    @APIResponse(
                            ...
                            @Content(
                                    ...
                                    example = ResponseBodyExamples.RESPONSE_200_SUPPORTED_BIZSTEPS)),
                    @APIResponse(
                            responseCode = "401",
    ...
    Uni<Response> getBizSteps(@Context SecurityIdentity securityIdentity,
                              @Valid
                              @RestQuery
    ...
    @Parameters(
            value = {
                    @Parameter(description = "", required = true, in = ParameterIn.PATH, example = "urn:epcglobal:cbv:bizstep:receiving"),
                    @Parameter(
                            name = "GS1-Extensions",
                            description = ParameterDescriptions.GS1_EXTENSIONS,
                            in = ParameterIn.HEADER)
    ...
    @Consumes({MediaType.APPLICATION_JSON, "application/ld+json"})
    Uni<Response> getSubResourceOfBizStep(@Context SecurityIdentity securityIdentity,
                                          @Context UriInfo uriInfo,@RestPath String bizStep);
    ...
    @Operation(
            ...
            description =
                    """
                            This endpoint helps to navigate EPCIS events by business steps. It returns EPCIS events up to the amount defined in `perPage`. The server returns a `Link` header to point to theremaining results.  Optionally, EPCIS events can be further filtered using the EPCIS Query Language in as querystring parameters.
    ...
    @Parameters(
            value = {
                    @Parameter(
                            description = ParameterDescriptions.BIZ_STEP,
                            required = true,
                            in = ParameterIn.PATH),
    ...
    @Consumes({MediaType.APPLICATION_JSON, "application/ld+json"})
    Uni<Response> getEventsFromBizSteps(
    ...
    @Produces({MediaType.APPLICATION_JSON, "application/problem+json", "application/ld+json"})
    @Consumes({MediaType.APPLICATION_JSON, "application/ld+json"})
    Uni<Response> getBizStepsOptions();
    ...
    @OPTIONS
    @Path("{bizStep}")
    Uni<Response> bizStepOptions(@RestPath String bizStep);
    ...
    @Consumes({MediaType.APPLICATION_JSON, "application/ld+json"})
    Uni<Response> bizStepEventsOptions(@RestPath String bizStep);
